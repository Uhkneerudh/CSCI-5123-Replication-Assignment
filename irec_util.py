...
def calc_scores(evaluator, interactions):
    ...
    #NEW CODE
    gini_values = evaluator.evaluate(metric_class=GiniCoefficientInv, results=interactions) 
    auc_values = []
    for x in gini_values:
        auc_values.append(np.mean([((y+1)/2) for y in x.values()]))
    ...
    #NEW CODE
    evaluation['auc'] = auc_values
    evaluation['p&r'] = evaluation['precision'] + evaluation['recall'] 
...
def calc_avg_scores(evaluator, interaction_set):
    ...
    #NEW CODE
    evaluation['auc'] = pd.concat([result[['auc']].T for result in results]).mean(axis=0)
    evaluation['p&r'] = pd.concat([result[['p&r']].T for result in results]).mean(axis=0)
